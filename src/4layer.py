        ...
        case "c":
            ...
            if order == 0:
                sols = [
                    sol.replace(
                        sp.exp,
                        lambda arg: sp.exp(arg.subs({x: x - sp.Function("h1")(z)})),
                    )
                    for sol in sols
                ]
...
if __name__ == "__main__":
    ...
    E_boundry_cl_0 = gen_boundry_conds(
        R, delop, E_vec_comps[0], border_func(h1), h1(z), ["c", "l"], order=0
    )

    H_boundry_cl_0 = gen_boundry_conds(
        R, delop, H_vec_comps[0], border_func(h1), h1(z), ["c", "l"], order=0
    )

    E_boundry_lf_0 = gen_boundry_conds(
        R, delop, E_vec_comps[0], border_func(h2), h2(z), ["l", "f"], order=0
    )

    H_boundry_lf_0 = gen_boundry_conds(
        R, delop, H_vec_comps[0], border_func(h2), h2(z), ["l", "f"], order=0
    )

    E_boundry_fs_0 = gen_boundry_conds(
        R, delop, E_vec_comps[0], border_func(h2), 0, ["f", "s"], order=0
    )

    H_boundry_fs_0 = gen_boundry_conds(
        R, delop, H_vec_comps[0], border_func(h2), 0, ["f", "s"], order=0
    )
    ...
    boundry_eqs_0 = list_subs(
        ...
        {
            eq.lhs: eq.rhs
            for eq in list_subs(sols_0_layers["c"] + sols_0_layers["l"], {x: h1(z)}, eval=False)
            +list_subs(sols_0_layers["l"] + sols_0_layers["f"], {x: h2(z)}, eval=False)
    ...
    # M_0_TE = M_0_TE.subs(symbolic_subs)
    print('getting det TE mode')
    det_TE = M_0_TE.subs(symbolic_subs).subs(numeric_parameters).doit()#.det().simplify()
    det_TE_numerical = sp.lambdify(beta, sp.im(det_TE), modules=[{'sqrt':np.emath.sqrt},'numpy'])

    quit()

    # M_0_TM = M_0_TM.subs(symbolic_subs)
    print('getting det TM mode')    
    det_TM = M_0_TM.subs(symbolic_subs).subs(numeric_parameters).doit().det().simplify()
    det_TM_numerical = sp.lambdify(beta, sp.im(det_TM), modules=[{'sqrt':np.emath.sqrt},'numpy'])

    beta_results = [
        bisect(det_TE_numerical, 1.54, 1.56, xtol=1e-16),
        bisect(det_TE_numerical, 1.50, 1.52, xtol=1e-16),
        bisect(det_TM_numerical, 1.54, 1.56, xtol=1e-16),
        bisect(det_TM_numerical, 1.50, 1.52, xtol=1e-16)
    ]

    # check beta with results from https://link.springer.com/chapter/10.1007/978-3-030-26831-2_8
    beta_previous_results = [1.5514927380692901290,
